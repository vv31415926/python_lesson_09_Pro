...
class GoGame:
    ...
    def win_gamer(self, ig, num=False):
        ...
        if self.slave_gamer == ig:   # к кому ходят
            print(' '*10 + 'Стол:'+self.win_table()+'   колода:'+str(self.k.get_count() ),'    Козырь:'+self.trump[1]+self.trump[2]  )
        else:
            print()
    ...
    def start(self):
        ...
        self.trump = self.k.card_out(trump=True)
        self.trump_color = self.trump[3]


        # Приведение к козырям
        for i in range( self.k.get_count() ):
            v = self.k.get_card(i)
            c = v[3]
            if c == self.trump_color:
                v[0] += self.trump_weight
                self.k.set_card(i,v)
            #print( self.k.get_card(i) )
        for i in range(len(self.gamers)):
